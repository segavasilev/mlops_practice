...
## Запуск скриптов
Для запска скриптов в среде Linux необходимо предоставить права на исполнение скрипта:
```shell
chmod +x pipeline.sh
```

Аналогичную операцию рекомендуется выполнить для всех python-скриптов:
```shell
chmod +x data_creation.py
chmod +x data_preprocessing.py
chmod +x model_preparation.py
chmod +x model_testing.py
```

## Возможные проблемы и их решения
При запуске скриптов следует учитывать, что имеются переменные, которые определены по умолчанию:
* Путь к папке с обучающими данными: "train"
* Имя файла с обучающими данными: "train.csv"
* Путь к папке с тестовыми данными: "test"
* Имя файла с тестовыми данными: "test.csv"
* Имя целевой переменной: "rhum"
* Путь к папке с обработанными обучающими данными: "train"
* Имя файла с обработанными обучающими данными: "prepared_train.csv"
* Путь к папке с обработанными тестовыми данными: "test"
* Имя файла с обработанными тестовыми данными: "prepared_test.csv"
* Путь к папке с моделью: "model"
* Имя файла с моделью: "model.pkl"

В случае, если необходимо изменить значения переменных, установленных по умолчанию, следует передать их в качестве аргументов при запуске отдельных скриптов, либо внести изменения в код баш-скрипта pipeline.sh.

Скрипты могут не запуститься, если необходимые импорты не могут быть выполнены. Так в скрипте model_testing.py имеется зависимость с кодом из файла model_preparation.py. И указанная проблема характерна для windows-систем. В данном случае рекомендуется запускать скрипты в среде Linux.